function Z = model_init_z(model, config)
    ...
    assert (config.num_primitive_params == 3);
    num_z_dim = config.num_primitives * config.num_primitive_params;
    
    mu = normrnd(1,0.1,num_z_dim,1).*repmat([rotation_guess;movement_guess;num_steps],config.num_primitives,1);
