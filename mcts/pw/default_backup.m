function default_backup(iter, nodes, trace, max_depth, compute_score)
...
for i = (max_depth-1):-1:1
    ...
    nodes(node_idx).visits(traj_idx) = nodes(node_idx).visits(traj_idx) + 1;
    nodes(node_idx).traj_score = compute_score(nodes(node_idx).traj_p_sum(traj_idx), nodes(node_idx).visits(traj_idx), iter);
end
