function [y, Sigma_y] = GMR(Priors, Mu, Sigma, invSigma, x, in, out)
%
...
nbData = size(x,2); % we need to use size for single vectors!
nbVar = size(Mu,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:nbStates
  Pxi(:,i) = Priors(i).*gaussPDF_fast(x, Mu(in,i), Sigma(in,in,i), invSigma(:,:,i));
end
beta = Pxi./repmat(sum(Pxi,2)+realmin,1,nbStates);
%% Compute expected means y, given input x
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j=1:nbStates
    % it seems like nbData is wrong -- but why would this only effect it
    % when I am recovering a lone example?
    % ------- CHRIS
    y_tmp(:,:,j) = repmat(Mu(out,j),1,nbData) + Sigma(out,in,j)*invSigma(:,:,j) * (x-repmat(Mu(in,j),1,nbData));
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for j=1:nbStates
  Sigma_y_tmp(:,:,1,j) = Sigma(out,out,j) - (Sigma(out,in,j)*invSigma(:,:,j)*Sigma(in,out,j));
end
